...
## Shorts 自動倍速の廃止
- `video_composer.py` の `compose_portrait()` から `with_speed_scaled()` を削除
- 3分超過時は警告メッセージを表示し、台本の手動削除を促す方式に変更
- 理由: 倍速によるピッチ上昇が不自然

## Shorts チャットアイコンの表情対応
- `compose_portrait()`: 固定の `normal_closed.png` → セリフごとの `{emotion}_closed.png` に変更
- `icon_cache` を `dict[tuple[str, str], Image.Image]` で表情ごとにキャッシュ
- `_render_chat_frame()` の引数を `tsuno_icon, megane_icon` → `icon_cache` に変更
- フォールバック: 対象表情の画像がなければ `normal` を使用

## YouTube アップロード invalidDescription エラー対応
- 原因: `note_content` 内の読みアノテーション `<にーさ>` 等を YouTube API が HTML タグとして拒否
- `upload.py`: `remove_reading_annotations()` を概要欄とタイトルに適用

## 使用料金の確認方法をマニュアルに追加
- `manual.md` に Gemini API / X API の課金方式・確認先の一覧を追加

## 読みアノテーションの数字対応
- `reading_annotations.py`: `_READING_PATTERN` と `_INLINE_PATTERN` の正規表現に `0-9０-９` を追加
- `881000<はちはちいちぜろぜろぜろ>` のような数字アノテーションが認識されるように

## COEIROINK ひらがな数詞の結合解釈問題
- 原因: COEIROINK の韻律推定が `はちはちいち` を数詞として結合し `はっぴゃくはちじゅういち` に変換
- 対策: 読みを**カタカナ+スペース区切り**にする（`ハチ ハチ イチ ゼロ ゼロ ゼロ`）
- `reading_dict.txt` と `script.json` のアノテーションを更新
- `manual.md` に注意書きを追加
- `audio_generator.py` に TTS 送信テキストの INFO ログを追加
